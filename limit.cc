...
void draw_limits(const lim_t & lims)
{
  ...
  slimconclusion.SetPoint(slimconclusion.GetN(), 1.00, ymax);

  slimconclusion.SetLineWidth(2);
  slimconclusion.SetLineStyle(kDashed);
  ...
  }
  {
    ...
    TLegend *l = new TLegend(macrox,      macroy,
                             macrox+0.15, macroy+3*textsize*1.02);
    l->SetTextSize(textsize);
    l->SetBorderSize(0);
    l->SetFillStyle(0);
    l->SetTextFont(42);
    ...
    l->AddEntry((TH1D*)NULL, "  underground", "");
